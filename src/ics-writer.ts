...
import { AddEventIntent } from './intent.js';

export function writeIcsFile(intent: AddEventIntent): string {
  ...
  if (intent.description) event.description = intent.description;
  if (intent.location) event.location = intent.location;
